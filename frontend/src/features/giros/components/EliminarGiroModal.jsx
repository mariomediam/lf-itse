...
/**
 * Modal de confirmación para eliminar un giro.
 *
 * Props
 * -----
 * isOpen    : bool
 * onClose   : () => void
 * onSuccess : () => void
 * giro      : object  — { id, nombre, ciiu_id }
 */
export default function EliminarGiroModal({ isOpen, onClose, onSuccess, giro }) {
  const [isSubmitting, setIsSubmitting] = useState(false)

  const handleConfirmar = async () => {
    setIsSubmitting(true)
    try {
      await girosApi.eliminar(giro.id)
      toast.success(`Giro "${giro.nombre}" eliminado correctamente`)
      onClose()
      onSuccess?.()
    } catch (err) {
    ...
    <Modal show={isOpen} size="md" onClose={onClose}>
      ...
      <ModalBody className="bg-white px-6 py-6 space-y-4">
        ...
        <div className="flex gap-3 rounded-lg bg-red-50 border border-red-200 px-4 py-3">
          ...
          <div className="text-sm text-red-700 space-y-1">
            ...
            <p>
              No se podrá eliminar si el giro está asignado a licencias de
              funcionamiento o certificados ITSE.
            </p>
          </div>
        </div>
      ...
      <ModalFooter className="border-t border-gray-200 bg-white flex justify-end gap-3">
        ...
        >
          ...
          {isSubmitting ? 'Eliminando...' : 'Sí, eliminar giro'}
        </button>
      </ModalFooter>
    </Modal>
  )

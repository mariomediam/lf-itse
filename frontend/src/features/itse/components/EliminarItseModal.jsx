...
/**
 * Modal de confirmación para eliminar una ITSE.
 *
 * Props
 * -----
 * isOpen    : bool
 ...
 * itse      : object      — fila de la ITSE (id, numero_itse, fecha_expedicion, nombre_comercial)
 */
export default function EliminarItseModal({ isOpen, onClose, onSuccess, itse }) {
  const [isSubmitting, setIsSubmitting] = useState(false)

  const numeroItse = itse
  ...
  const handleConfirmar = async () => {
    ...
    try {
      await itseApi.eliminar(itse.id)
      toast.success(`${numeroItse} eliminada correctamente`)
      onClose()
      onSuccess?.()
    } catch (err) {
    ...
    <Modal show={isOpen} size="md" onClose={onClose}>
      ...
      <ModalBody className="bg-white px-6 py-6 space-y-4">
        ...
        <div className="flex gap-3 rounded-lg bg-red-50 border border-red-200 px-4 py-3">
          ...
          <div className="text-sm text-red-700 space-y-1">
            ...
            <p>
              Se eliminarán permanentemente la ITSE, sus estados,
      ...
      <ModalFooter className="border-t border-gray-200 bg-white flex justify-end gap-3">
        ...
        >
          ...
          {isSubmitting ? 'Eliminando...' : 'Sí, eliminar ITSE'}
        </button>
      </ModalFooter>
    </Modal>
  )

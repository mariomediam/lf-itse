...
/**
 * Modal de confirmación para eliminar un expediente.
 *
 * Props
 * -----
 * isOpen     : bool
 ...
 * expediente : object      — fila del expediente (id, numero_expediente, fecha_recepcion, nombre_procedimiento)
 */
export default function EliminarExpedienteModal({ isOpen, onClose, onSuccess, expediente }) {
  ...
  const handleConfirmar = async () => {
    ...
    try {
      await expedientesApi.eliminar(expediente.id)
      toast.success(`Expediente ${numeroExpediente} eliminado correctamente`)
      onClose()
      onSuccess?.()
    } catch (err) {
    ...
    <Modal show={isOpen} size="md" onClose={onClose}>
      ...
      <ModalBody className="bg-white px-6 py-6 space-y-4">
        ...
        <div className="flex gap-3 rounded-lg bg-red-50 border border-red-200 px-4 py-3">
          ...
          <div className="text-sm text-red-700 space-y-1">
            ...
            <p>
              Se eliminarán permanentemente el expediente, sus autorizaciones
              improcedentes y todos los documentos adjuntos.
            </p>
          </div>
        </div>
      ...
      <ModalFooter className="border-t border-gray-200 bg-white flex justify-end gap-3">
        ...
        >
          ...
          {isSubmitting ? 'Eliminando...' : 'Sí, eliminar expediente'}
        </button>
      </ModalFooter>
    </Modal>
  )

...
/**
 * Modal para gestionar los archivos adjuntos de un expediente.
 *
 * Props
 * -----
 * isOpen     : bool
 ...
 */
export default function DocumentosAdjuntosModal({ isOpen, onClose, expediente }) {
  const [archivos,           setArchivos]           = useState([])
  const [cargando,           setCargando]           = useState(false)
  const [archivoSeleccionado, setArchivoSeleccionado] = useState(null)
  const [subiendo,           setSubiendo]           = useState(false)
  const [confirmandoId,      setConfirmandoId]      = useState(null) // id en confirmación de borrado
  ...
  const cargarArchivos = useCallback(async () => {
    ...
    setCargando(true)
    try {
      const res = await expedientesApi.listarArchivos(expediente.id)
      setArchivos(res.data)
    } catch {
      toast.error('Error al cargar los archivos del expediente')
    } finally {
      setCargando(false)
    }
  }, [expediente])
  ...
  const handleSubir = async () => {
    ...
    setSubiendo(true)
    try {
      await expedientesApi.subirArchivo(expediente.id, formData)
  ...
  const handleConfirmarEliminar = async (archivo) => {
    ...
    try {
      await expedientesApi.eliminarArchivo(archivo.id)

...
/**
 * Modal de confirmación para eliminar una zonificación.
 *
 * Props
 * -----
 * isOpen       : bool
 * onClose      : () => void
 * onSuccess    : () => void
 * zonificacion : object  — { id, codigo, nombre }
 */
export default function EliminarZonificacionModal({ isOpen, onClose, onSuccess, zonificacion }) {
  const [isSubmitting, setIsSubmitting] = useState(false)

  const handleConfirmar = async () => {
    setIsSubmitting(true)
    try {
      await zonificacionesApi.eliminar(zonificacion.id)
      toast.success(`Zonificación "${zonificacion.nombre}" eliminada correctamente`)
      onClose()
      onSuccess?.()
    } catch (err) {
      ...
      const msg =
        ...
        'Error al eliminar la zonificación'
      toast.error(msg)
    } finally {
    ...
    <Modal show={isOpen} size="md" onClose={onClose}>
      ...
      <ModalBody className="bg-white px-6 py-6 space-y-4">
        ...
        <div className="flex gap-3 rounded-lg bg-red-50 border border-red-200 px-4 py-3">
          ...
          <div className="text-sm text-red-700 space-y-1">
            ...
            <p>
              No se podrá eliminar si la zonificación está asignada a
      ...
      <ModalFooter className="border-t border-gray-200 bg-white flex justify-end gap-3">
        ...
        >
          ...
          {isSubmitting ? 'Eliminando...' : 'Sí, eliminar zonificación'}
        </button>
      </ModalFooter>
    </Modal>
  )

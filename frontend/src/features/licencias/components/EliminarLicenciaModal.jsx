...
/**
 * Modal de confirmación para eliminar una licencia de funcionamiento.
 *
 * Props
 * -----
 * isOpen    : bool
 * onClose   : () => void
 * onSuccess : () => void  — se llama tras eliminar para refrescar la lista
 * licencia  : object      — fila de la licencia (id, numero_licencia, fecha_emision, nombre_comercial)
 */
export default function EliminarLicenciaModal({ isOpen, onClose, onSuccess, licencia }) {
  const [isSubmitting, setIsSubmitting] = useState(false)

  const numeroLicencia = licencia
    ? formatNumeroLicencia(licencia.numero_licencia, licencia.fecha_emision)
    : ''

  const handleConfirmar = async () => {
    setIsSubmitting(true)
    try {
      await licenciasApi.eliminar(licencia.id)
      toast.success(`Licencia de funcionamiento ${numeroLicencia} eliminada correctamente`)
      onClose()
      onSuccess?.()
    } catch (err) {
      ...
      const msg =
        ...
        'Error al eliminar la licencia de funcionamiento'
      toast.error(msg)
    } finally {
    ...
    <Modal show={isOpen} size="md" onClose={onClose}>
      ...
      <ModalHeader className="bg-white border-b border-gray-200">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-base font-semibold text-gray-800">
            Eliminar licencia de funcionamiento
          </span>
        </div>
      </ModalHeader>
      ...
      <ModalBody className="bg-white px-6 py-6 space-y-4">
        ...
        <p className="text-xl font-bold text-gray-800">
          Licencia de funcionamiento {numeroLicencia}
        </p>
        ...
        <div className="flex gap-3 rounded-lg bg-red-50 border border-red-200 px-4 py-3">
          ...
          <div className="text-sm text-red-700 space-y-1">
            ...
            <p>
              Se eliminarán permanentemente la licencia de funcionamiento, sus estados,
              giros y todos los documentos adjuntos.
            </p>
          </div>
        </div>
      ...
      <ModalFooter className="border-t border-gray-200 bg-white flex justify-end gap-3">
        ...
        >
          ...
          {isSubmitting ? 'Eliminando...' : 'Sí, eliminar licencia'}
        </button>
      </ModalFooter>
    </Modal>
  )

...
/**
 * Modal inicial para agregar una licencia de funcionamiento.
 ...
 */
export default function AgregarLicenciaModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleContinuar = async () => {
    ...
    setLoading(true)
    try {
      const res = await licenciasApi.verificarExpediente(
        Number(numeroExpediente),
        Number(anio),
      )
      const { se_puede_emitir_licencia, expediente_id, mensaje } = res.data

      if (!se_puede_emitir_licencia) {
        toast.error(mensaje)
        return
      }
    ...
    <Modal show={isOpen} size="md" onClose={handleClose}>
      ...
      <ModalBody className="bg-white px-6 py-6 space-y-5">
        <p className="text-sm text-gray-500">
          Ingrese los datos del expediente para iniciar el registro de la
